...
class LoginPage:
    ...
    def get_email_validation_message(self) -> str:
        """Returns the HTML5 validation message on the email input field."""
        try:
            return self.page.locator(self.txtUsername).first.evaluate("el => el.validationMessage")
        except Exception:
            return ""

    def get_password_validation_message(self) -> str:
        """Returns the HTML5 validation message on the password input field."""
        try:
            return self.page.locator(self.txtPassword).first.evaluate("el => el.validationMessage")
        except Exception:
            return ""

    def login_with_empty_fields(self, username: str = "", password: str = ""):
        """Enters username/password if provided (or clears if empty) and clicks submit."""
        user_loc = self.page.locator(self.txtUsername).first
        user_loc.wait_for(state="visible", timeout=10000)
        user_loc.fill(username if username else "")

        pass_loc = self.page.locator(self.txtPassword).first
        pass_loc.wait_for(state="visible", timeout=5000)
        pass_loc.fill(password if password else "")
    ...
    getEmailValidationMessage = get_email_validation_message
    getPasswordValidationMessage = get_password_validation_message
