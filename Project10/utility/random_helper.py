...
class RandomHelper:
    ...
    def get_random_card_details(cls, customer_name: str = None) -> dict:
        ...
        # Realistic mock card brand prefix (Visa / Mastercard)
        brand_prefix = random.choice(["4111", "4242", "5105", "5555"])
        card_num_suffix = f"{random.randint(100000000000, 999999999999)}"
        card_number = f"{brand_prefix}{card_num_suffix}"
    ...
    @classmethod
    def pick_random_brand(cls, brands: list, exclude_brand_name: str = None) -> dict:
        """Picks a random brand dictionary from a list of available brands."""
        if not brands:
            return {}
        candidates = brands
        if exclude_brand_name:
            candidates = [b for b in brands if b.get("name", "").lower() != exclude_brand_name.lower()]
            if not candidates:
                candidates = brands
        return random.choice(candidates)

    # Aliases for exact Java compatibility
    getRandomQuantity = get_random_quantity
    pickRandomIndex = pick_random_index
    getRandomCustomer = get_random_customer
    getRandomOrderComment = get_random_order_comment
    getRandomCardDetails = get_random_card_details
    pickRandomBrand = pick_random_brand
